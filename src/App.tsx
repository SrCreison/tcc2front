...
function App() {
  ...
  useEffect(() => {
    console.log("LOG DE SUCESSO: O APP NOVO CARREGOU!");
  }, []);
  ...
  return (
    <div style={{ backgroundColor: '#020617', color: 'white', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px', fontFamily: 'sans-serif' }}>
      ...
      {/* SE VOCÊ NÃO VIR ESSE QUADRADO VERMELHO, O BUILD ESTÁ ERRADO */}
      <div style={{ background: 'red', color: 'white', padding: '10px', fontWeight: 'bold', marginBottom: '20px' }}>
        TESTE DE ATUALIZAÇÃO: V2 ATIVA
      </div>
      ...
      <div style={{ textAlign: 'center' }}>
        <p style={{ fontWeight: 'bold', marginBottom: '15px' }}>{status}</p>
        <button onClick={() => girarRoleta(false)} disabled={loading} style={{ padding: '15px 30px', margin: '5px', cursor: 'pointer', backgroundColor: '#6366f1', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold' }}>
          {loading ? '...' : 'JOGAR R$ 2'}
        </button>
      </div>
    </div>
